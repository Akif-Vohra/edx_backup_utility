...
format = '%Y-%m-%d-%H:%M'
t = datetime.datetime.now().strftime(format)
...
MYSQL_BACKUP_FILE_NAME = 'backup.sql{0}'.format(t)
...
MONGO_BACKUP_FILE_NAME = 'mongo-backup{0}'.format(t)
...
mongo_backup_zip_name = os.path.join(MONGO_BACKUP_DIR, 'MONGO_BACKUP_{0}'.format(t))
...
ZIP_FILE = os.path.join(MEDIA_ZIP_DIR, 'media_{0}'.format(t))

print "Zip media directory"
shutil.make_archive(ZIP_FILE, 'zip', media_zip_name)

#Step 4 : Send to S3
S3_BUCKET_NAME = 'lynx-edx-learning-platform'
